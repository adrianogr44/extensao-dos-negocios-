...
import { disconnectSession } from '@/lib/youtube-publisher/session'
...
export async function GET(
  ...
  try {
    const session = await prisma.youtubeSession.findUnique({
      ...
      select: { id: true, status: true, loggedInAs: true, googleUserId: true, lastUsedAt: true, youtubeAccountId: true },
    })
...
export async function DELETE(
  ...
  try {
    ...
    if (!session || !session.youtubeAccountId) {
      return NextResponse.json({ success: false, error: 'Sessão não encontrada' }, { status: 404 })
    }

    await disconnectSession(session.youtubeAccountId)

    return NextResponse.json({ success: true })
  } catch (err) {
    console.error('[YT Session Delete]', err)
